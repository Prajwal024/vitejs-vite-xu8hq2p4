...
@keyframes splashSub{0%{opacity:0;transform:translateY(20px)}100%{opacity:1;transform:translateY(0)}}
...
.splash-title{font-family:'Outfit',sans-serif;font-weight:900;font-size:clamp(28px,6vw,42px);color:#fff;letter-spacing:-.01em;text-align:center;opacity:0;animation:splashText .6s ease .5s forwards;line-height:1.1}
.splash-title span{color:var(--green)}
.splash-tagline{font-size:13px;font-weight:600;color:var(--muted2);letter-spacing:.15em;text-transform:uppercase;margin-top:10px;opacity:0;animation:splashTagline .8s ease .9s forwards;text-align:center}
...
.splash-bar{height:100%;background:linear-gradient(90deg,var(--green),#a78bfa,var(--green));background-size:200% auto;border-radius:2px;animation:splashBar 1.4s cubic-bezier(.4,0,.2,1) 1.1s forwards,shimmer 2s linear 1.1s infinite}
...
.nav-logo:hover .nav-icon{transform:scale(1.1);box-shadow:0 0 0 4px rgba(34,197,94,.25)}
.nav-brand{font-family:'Outfit',sans-serif;font-weight:800;font-size:16px;color:var(--text)}
.nav-brand span{color:var(--green)}
.nav-tabs{display:flex;gap:2px}
.nav-tab{padding:6px 13px;border-radius:8px;border:none;background:transparent;cursor:pointer;font-family:'DM Sans',sans-serif;font-size:13px;font-weight:500;color:var(--muted);transition:all .18s;position:relative;overflow:hidden}
.nav-tab::after{content:'';position:absolute;bottom:0;left:50%;transform:translateX(-50%);width:0;height:2px;background:var(--green);border-radius:2px;transition:width .2s}
...
.nav-av{width:32px;height:32px;border-radius:50%;background:var(--green-bg);border:1.5px solid var(--green-b);display:flex;align-items:center;justify-content:center;font-weight:700;font-size:11px;color:var(--green);font-family:'Outfit',sans-serif;animation:ringPulse 3s ease 3s infinite;transition:transform .2s}
...
.btn-p:hover{background:linear-gradient(135deg,#16a34a,#15803d);transform:translateY(-2px);box-shadow:0 6px 20px rgba(34,197,94,.4)}
...
.fi:focus{border-color:var(--green);box-shadow:0 0 0 3px rgba(34,197,94,.15);animation:inputGlow .6s ease}
...
.fta{width:100%;padding:10px 14px;background:var(--s2);border:1.5px solid var(--border);border-radius:9px;color:var(--text);font-family:'DM Sans',sans-serif;font-size:13px;outline:none;resize:vertical;min-height:70px;transition:border .18s,box-shadow .18s}
...
.num-input:focus-within{border-color:var(--green);box-shadow:0 0 0 3px rgba(34,197,94,.12)}
...
.auth-title{font-family:'Outfit',sans-serif;font-weight:800;font-size:24px;text-align:center;margin-bottom:4px;animation:fadeUp .5s ease .1s both}
.auth-sub{font-size:13px;color:var(--muted);text-align:center;margin-bottom:28px;animation:fadeUp .5s ease .2s both}
.auth-btn{width:100%;padding:13px;background:linear-gradient(135deg,#22c55e,#16a34a);color:#fff;border:none;border-radius:10px;font-family:'Outfit',sans-serif;font-weight:700;font-size:15px;cursor:pointer;margin-top:4px;box-shadow:0 4px 16px rgba(34,197,94,.25);transition:all .2s;position:relative;overflow:hidden}
.auth-btn::after{content:'';position:absolute;inset:0;background:linear-gradient(105deg,transparent 40%,rgba(255,255,255,.2) 50%,transparent 60%);background-size:200% auto;opacity:0}
.auth-btn:hover{box-shadow:0 6px 24px rgba(34,197,94,.4);transform:translateY(-2px)}
.auth-btn:hover::after{opacity:1;animation:shimmer .5s ease}
...
.photo-item img,.photo-item video{width:100%;height:100%;object-fit:cover;transition:transform .3s}
...
.sources-table-wrap{background:var(--s3);border:1px solid var(--border);border-radius:10px;padding:12px 14px;margin-bottom:16px;animation:fadeUp .3s ease forwards}
...
.slider-val{font-family:'Outfit',sans-serif;font-size:20px;font-weight:800;color:var(--green);min-width:28px;text-align:right;transition:transform .15s}
.slider-wrap:hover .slider-val{transform:scale(1.1)}
.slider{-webkit-appearance:none;appearance:none;width:100%;height:6px;border-radius:3px;background:var(--s3);outline:none;cursor:pointer;transition:height .15s}
.slider:hover{height:8px}
...
function WorkoutEditor({ plan, onSave, onClose, autoSave }) {
  ...
  const updateDay = (di, f, v) => { const u = days.map((day, i) => i === di ? { ...day, [f]: v } : day); setDays(u); triggerAutoSave(u); };
  const updateEx = (di, ei, f, v) => { const u = days.map((day, i) => i === di ? { ...day, exercises: day.exercises.map((ex, j) => j === ei ? { ...ex, [f]: v } : ex) } : day); setDays(u); triggerAutoSave(u); };
  const addEx = (di) => { const u = days.map((day, i) => i === di ? { ...day, exercises: [...day.exercises, { name: "", sets: 3, reps: "10-12", rest: "60s", videoUrl: "", note: "" }] } : day); setDays(u); triggerAutoSave(u); };
  const removeEx = (di, ei) => { const u = days.map((day, i) => i === di ? { ...day, exercises: day.exercises.filter((_, j) => j !== ei) } : day); setDays(u); triggerAutoSave(u); };
...
function MealEditor({ plan, onSave, onClose, clientSources, autoSave }) {
  ...
  const triggerAutoSave = (newMeals) => {
    if (saveTimer.current) clearTimeout(saveTimer.current);
    saveTimer.current = setTimeout(() => { autoSave(newMeals); setAutoSaved(true); setTimeout(() => setAutoSaved(false), 2000); }, 1000);
  };
        ...
        <div className="card" style={{ marginBottom: 14 }}>
          ...
          <div style={{ borderTop: "1px solid var(--border)", paddingTop: 14 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
              {[["From Protein", calFromP, "var(--purple)", totCal], ["From Carbs", calFromC, "var(--orange)", totCal], ["From Fats", calFromF, "var(--red)", totCal]].map(([l, v, co, tot], i) => {
      ...
      <div className="page">
        ...
        {viewMedia && (
          <div className="ov" onClick={() => setViewMedia(null)}>
            <div style={{ maxWidth: 560, width: "100%", animation: "bounceIn .4s ease forwards" }}>
  ...
  return (
    <div className="page">
      <div style={{ marginBottom: 22 }}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginTop: 10, alignItems: "center", animation: "fadeUp .5s ease .1s both" }}>
          <span className="phase">{d.phase} - Week {d.week}</span>
          <span className="live"><span className="dot" />Live sync</span>
        </div>
      </div>
      <div className="g4" style={{ marginBottom: 18 }}>
        {[["Weight KG", d.weight, "var(--green)", !!flash.weight], ["Waist CM", d.waist, "var(--purple)", !!flash.waist], ["Body Fat %", d.bodyFat, "var(--orange)", !!flash.bodyFat], ["Week", "W" + d.week, "var(--blue)", false]].map(([l, v, co, fl], i) => (
          <div key={l} style={{ animationDelay: i * 0.08 + "s" }}><MC label={l} value={v} color={co} flash={fl} /></div>
        ))}
      </div>
      ...
      <div className={`stagger-4 ${flash.msg ? "card flash" : "card"}`} style={{ marginBottom: 16 }}>
      ...
      {(d.weightHistory || []).length > 1 && (
        <><div className="sh"><div className="sh-title">Weight Progress</div></div>
          <div className="card stagger-6" style={{ marginBottom: 24 }}>
...
function CoachDash({ coachUid, coachEmail, coachName, tab, setTab, toast }) {
  ...
  if (tab === "analytics") return (
    <div className="page">
      <div style={{ fontFamily: "'Outfit',sans-serif", fontSize: 22, fontWeight: 800, marginBottom: 22, animation: "fadeDown .4s ease forwards" }}>Analytics</div>
      <div className="g2">
        {clients.map((c, i) => (
    ...
    return (
      <div className="page">
              ...
              <div className="card stagger-2" style={{ marginBottom: 14 }}>
                ...
                <div className="g3" style={{ marginBottom: 10 }}>
                  {[["Stress", todayCheckin.stressLevel, "#f87171"], ["Hunger", todayCheckin.hungerCravings, "#fb923c"], ["Training", todayCheckin.trainingPerformance, "#4ade80"], ["Nutrition", todayCheckin.nutritionAdherence, "#a78bfa"], ["Sleep", todayCheckin.sleepQuality, "#38bdf8"], ["Water", todayCheckin.waterIntake + "L", "#38bdf8"]].map(([l, v, co], i) => (
                    <div key={l} style={{ background: "var(--s2)", borderRadius: 9, padding: "10px 12px", border: "1px solid var(--border)", textAlign: "center", animation: "bounceIn .4s ease " + (i * 0.06) + "s both" }}>
                ...
                {todayCheckin.noteToCoach && <div className="note-box">{todayCheckin.noteToCoach}</div>}
              </div>
            )}
        ...
        {innerTab === "analytics" && (() => {
          ...
          const totP = allItems.reduce((a, i) => a + (i.protein || 0), 0); const totC = allItems.reduce((a, i) => a + (i.carbs || 0), 0);
          const totF = allItems.reduce((a, i) => a + (i.fats || 0), 0); const totFib = allItems.reduce((a, i) => a + (i.fiber || 0), 0);
          const totCal = allItems.reduce((a, i) => a + (i.cal || 0), 0); const nn = sel.nutrition || {};
          return (
            <div>
        ...
        {innerTab === "sources" && (() => {
          ...
          return (
            <div className="card">
              ...
              {[["protein", "Protein Sources", "var(--purple)"], ["carbs", "Carb Sources", "var(--orange)"], ["fats", "Fat Sources", "var(--red)"]].map(([type, label, color], ci) => (
        ...
        {viewMedia && (
          <div className="ov" onClick={() => setViewMedia(null)}>
            <div style={{ maxWidth: 520, width: "100%", animation: "bounceIn .4s ease forwards" }}>
  ...
  if (tab === "clients") return (
    <div className="page">
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20, animation: "fadeDown .4s ease forwards" }}>
  ...
  return (
    <div className="page">
      <div style={{ marginBottom: 24, animation: "fadeDown .5s ease forwards" }}>
        ...
        : <>
          ...
          <div className="ga">
            {clients.map((c, idx) => {
              const todayDaily = (c.dailyCheckins || []).find(dc => dc.date === new Date().toLocaleDateString("en-IN"));
              return (
                <div key={c.id} className="cl-card" style={{ animationDelay: idx * 0.07 + "s" }} onClick={() => { setTab("clients"); setSelId(c.id); setInnerTab("overview"); }}>
...
function LoginScreen({ onLogin, onSetup, coachExists }) {
  ...
  return (
    <div className="auth-wrap"><div className="auth-card">
      ...
      <div style={{ textAlign: "right", marginTop: 8, animation: "fadeUp .5s ease .45s both" }}><button style={{ background: "none", border: "none", cursor: "pointer", fontSize: 11, color: "var(--muted)", textDecoration: "underline", transition: "color .15s" }} onMouseOver={e => e.target.style.color = "var(--green)"} onMouseOut={e => e.target.style.color = "var(--muted)"} onClick={() => setShowForgot(true)}>Forgot password?</button></div>
      {!coachExists && <div className="auth-switch" style={{ animation: "fadeUp .5s ease .5s both" }}>First time? <button onClick={onSetup}>Create coach account</button></div>}
      {coachExists && <div style={{ textAlign: "center", marginTop: 14, fontSize: 11, color: "var(--muted)", padding: 10, background: "var(--s2)", borderRadius: 10, border: "1px solid var(--border)", animation: "fadeUp .5s ease .5s both" }}>Contact Ankit for your login credentials.</div>}
    </div></div>
  );
}
