...
/* Loading logo pulse */
@keyframes logoPulse {
  0%,100% { box-shadow: 0 0 0 0 rgba(34,197,94,.5), 0 8px 24px rgba(34,197,94,.3); transform: scale(1); }
  50%      { box-shadow: 0 0 0 14px rgba(34,197,94,.0), 0 8px 32px rgba(34,197,94,.5); transform: scale(1.04); }
}
...
@keyframes flashBorder {
  0%,40% { border-color:var(--green); box-shadow:0 0 0 2px rgba(34,197,94,.2); }
  100%   { border-color:var(--border); box-shadow:none; }
}
/* Wk card glow on hover */
@keyframes glowPulse {
  0%,100% { box-shadow: 0 0 0 0 rgba(34,197,94,.3); }
  50%     { box-shadow: 0 0 12px 3px rgba(34,197,94,.15); }
}
/* Spinner */
...
/* Slide underline for nav tab */
@keyframes tabActivate {
  from { opacity:0; transform:scaleX(0); }
  to   { opacity:1; transform:scaleX(1); }
}
/* Card stagger — apply via inline style animationDelay */
.stagger { animation: fadeIn .4s ease both; }

/* ═══════════════════════════════════════════
   NAV
═══════════════════════════════════════════ */
.nav{background:rgba(8,13,26,.95);backdrop-filter:blur(16px);border-bottom:1px solid var(--border);padding:0 20px;height:58px;display:flex;align-items:center;justify-content:space-between;position:sticky;top:0;z-index:100;animation:fadeSlideUp .35s ease both;}
.nav-logo{display:flex;align-items:center;gap:9px;cursor:pointer;border:none;background:none;transition:transform .2s;}
.nav-logo:hover{transform:scale(1.03);}
.nav-icon{width:34px;height:34px;border-radius:10px;background:linear-gradient(135deg,#22c55e,#16a34a);display:flex;align-items:center;justify-content:center;font-family:'Outfit',sans-serif;font-weight:900;font-size:11px;color:#fff;letter-spacing:-.5px;transition:box-shadow .2s;}
.nav-logo:hover .nav-icon{box-shadow:0 0 16px rgba(34,197,94,.45);}
.nav-brand{font-family:'Outfit',sans-serif;font-weight:800;font-size:16px;color:var(--text);}
.nav-brand span{color:var(--green);}
.nav-tabs{display:flex;gap:2px;}
.nav-tab{padding:6px 13px;border-radius:8px;border:none;background:transparent;cursor:pointer;font-family:'DM Sans',sans-serif;font-size:13px;font-weight:500;color:var(--muted);transition:all .2s;position:relative;}
...
.signout{padding:5px 12px;border-radius:8px;border:1px solid var(--border);background:transparent;cursor:pointer;font-size:12px;font-weight:500;color:var(--muted);transition:all .2s;}
.signout:hover{border-color:var(--red);color:var(--red);}

/* ═══════════════════════════════════════════
   PAGE WRAPPER — fade in on mount
═══════════════════════════════════════════ */
.page{max-width:960px;margin:0 auto;padding:24px 16px 48px;animation:fadeSlideUp .4s ease both;}

/* ═══════════════════════════════════════════
   CARDS
═══════════════════════════════════════════ */
.card{background:var(--s1);border:1px solid var(--border);border-radius:var(--r);padding:18px;box-shadow:var(--sh);transition:border-color .2s, box-shadow .2s;}
...
/* ═══════════════════════════════════════════
   METRIC CARD — numPop on mount
═══════════════════════════════════════════ */
.mc{background:var(--s1);border:1px solid var(--border);border-radius:var(--r);padding:16px;transition:transform .2s, box-shadow .2s;}
.mc:hover{transform:translateY(-2px);box-shadow:0 8px 28px rgba(0,0,0,.5);}
.mc.flash{animation:flashBorder 2.5s ease forwards;}
.mc-val{font-family:'Outfit',sans-serif;font-size:28px;font-weight:800;line-height:1;animation:numPop .5s cubic-bezier(.34,1.56,.64,1) both;}
...
═══════════════════════════════════════════ */
.btn{display:inline-flex;align-items:center;gap:6px;padding:9px 16px;border-radius:var(--r);border:none;cursor:pointer;font-family:'DM Sans',sans-serif;font-size:13px;font-weight:600;transition:all .15s;white-space:nowrap;position:relative;overflow:hidden;}
.btn:active{animation:btnPress .2s ease forwards;}
.btn-p{background:var(--green);color:#fff;box-shadow:0 2px 12px rgba(34,197,94,.25);}
.btn-p:hover{background:#16a34a;box-shadow:0 4px 20px rgba(34,197,94,.4);transform:translateY(-1px);}
...
.fi{width:100%;padding:10px 14px;background:var(--s2);border:1.5px solid var(--border);border-radius:9px;color:var(--text);font-family:'DM Sans',sans-serif;font-size:14px;outline:none;transition:border .2s, box-shadow .2s;}
.fi:focus{border-color:var(--green);box-shadow:0 0 0 3px rgba(34,197,94,.12);}
.fi::placeholder{color:var(--muted);}
.fsel{width:100%;padding:10px 14px;background:var(--s2);border:1.5px solid var(--border);border-radius:9px;color:var(--text);font-family:'DM Sans',sans-serif;font-size:13px;outline:none;transition:border .2s;}
.fsel:focus{border-color:var(--green);}
.fta{width:100%;padding:10px 14px;background:var(--s2);border:1.5px solid var(--border);border-radius:9px;color:var(--text);font-family:'DM Sans',sans-serif;font-size:13px;outline:none;resize:vertical;min-height:70px;transition:border .2s, box-shadow .2s;}
...
.tab-item{flex:1;padding:7px 6px;border-radius:8px;border:none;background:transparent;cursor:pointer;font-family:'DM Sans',sans-serif;font-size:12px;font-weight:600;color:var(--muted);transition:all .2s;min-width:70px;text-align:center;}
...
.sh-link{font-size:12px;font-weight:600;color:var(--green);background:none;border:none;cursor:pointer;transition:opacity .15s;}
.sh-link:hover{opacity:.75;}

/* ═══════════════════════════════════════════
   MODAL / OVERLAY
═══════════════════════════════════════════ */
.ov{position:fixed;inset:0;background:rgba(0,0,0,.85);backdrop-filter:blur(8px);z-index:200;display:flex;align-items:center;justify-content:center;padding:16px;animation:fadeIn .2s ease both;}
.modal{background:var(--s1);border:1px solid var(--border2);border-radius:var(--r2);width:100%;max-width:640px;max-height:92vh;overflow-y:auto;box-shadow:var(--sh2);animation:mIn .25s cubic-bezier(.34,1.2,.64,1) both;}
...
.xbtn{width:28px;height:28px;border-radius:7px;background:var(--s2);border:1px solid var(--border);cursor:pointer;display:flex;align-items:center;justify-content:center;color:var(--muted);font-size:14px;flex-shrink:0;transition:all .15s;}
.xbtn:hover{color:var(--red);border-color:var(--red);transform:rotate(90deg);}
...
/* ═══════════════════════════════════════════
   TOAST — animated slide-in from right
═══════════════════════════════════════════ */
.toast{position:fixed;bottom:22px;right:22px;padding:12px 18px;border-radius:12px;font-size:13px;font-weight:600;z-index:9999;box-shadow:var(--sh2);animation:toastIn .3s cubic-bezier(.34,1.3,.64,1) both;max-width:320px;pointer-events:none;display:flex;align-items:center;gap:8px;}
.toast::before{content:'';width:6px;height:6px;border-radius:50%;flex-shrink:0;}
.toast-s{background:linear-gradient(135deg,#14532d,#166534);border:1px solid #22c55e55;color:#bbf7d0;}
.toast-s::before{background:#22c55e;}
.toast-e{background:linear-gradient(135deg,#450a0a,#7f1d1d);border:1px solid #f8717155;color:#fecaca;}
.toast-e::before{background:#f87171;}

/* ═══════════════════════════════════════════
   AUTH — animated card entrance
═══════════════════════════════════════════ */
.auth-wrap{min-height:100vh;display:flex;align-items:center;justify-content:center;padding:20px;background:radial-gradient(ellipse at 30% 20%,rgba(34,197,94,.07) 0%,transparent 60%),radial-gradient(ellipse at 70% 80%,rgba(167,139,250,.07) 0%,transparent 60%),var(--bg);}
.auth-card{background:var(--s1);border:1px solid var(--border2);border-radius:22px;padding:36px;width:100%;max-width:400px;box-shadow:var(--sh2);animation:fadeSlideUp .5s cubic-bezier(.34,1.1,.64,1) both;}
.auth-logo{width:56px;height:56px;border-radius:16px;background:linear-gradient(135deg,#22c55e,#16a34a);display:flex;align-items:center;justify-content:center;font-family:'Outfit',sans-serif;font-weight:900;font-size:17px;color:#fff;margin:0 auto 14px;animation:logoPulse 2.2s ease infinite;}
...
.auth-btn{width:100%;padding:13px;background:linear-gradient(135deg,#22c55e,#16a34a);color:#fff;border:none;border-radius:10px;font-family:'Outfit',sans-serif;font-weight:700;font-size:15px;cursor:pointer;margin-top:4px;box-shadow:0 4px 16px rgba(34,197,94,.3);transition:all .2s;}
.auth-btn:hover{box-shadow:0 8px 28px rgba(34,197,94,.45);transform:translateY(-2px);}
...
.auth-switch button{background:none;border:none;cursor:pointer;color:var(--green);font-size:12px;font-weight:600;transition:opacity .15s;}
...
/* ═══════════════════════════════════════════
   MESSAGE BOX
═══════════════════════════════════════════ */
.msg-b{background:var(--s2);border:1px solid var(--border);border-radius:12px;padding:13px 15px;font-size:13px;line-height:1.65;color:var(--muted2);white-space:pre-wrap;transition:all .3s;}
.msg-b.has{background:var(--green-bg);border-color:var(--green-b);color:var(--text);}
.nbadge{display:inline-flex;padding:2px 7px;border-radius:20px;font-size:10px;font-weight:700;background:var(--green-bg);color:var(--green);border:1px solid var(--green-b);margin-left:8px;animation:numPop .4s ease both;}

/* ═══════════════════════════════════════════
   LOADING SCREEN
═══════════════════════════════════════════ */
.spin-wrap{display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:300px;gap:16px;color:var(--muted);font-size:13px;}
.spinner{width:24px;height:24px;border-radius:50%;border:2.5px solid var(--border);border-top-color:var(--green);animation:sp .7s linear infinite;}
.loading-logo{width:64px;height:64px;border-radius:18px;background:linear-gradient(135deg,#22c55e,#16a34a);display:flex;align-items:center;justify-content:center;font-family:'Outfit',sans-serif;font-weight:900;font-size:18px;color:#fff;animation:logoPulse 1.8s ease infinite;}
.loading-text{font-family:'Outfit',sans-serif;font-weight:700;font-size:16px;color:var(--muted2);animation:pulse 1.8s ease infinite;}

/* ═══════════════════════════════════════════
   EMPTY STATE
═══════════════════════════════════════════ */
.empty{text-align:center;padding:40px 24px;color:var(--muted);animation:fadeIn .4s ease both;}
...
/* ═══════════════════════════════════════════
   WORKOUT CARDS — glow on hover
═══════════════════════════════════════════ */
.wk-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(130px,1fr));gap:10px;}
.wk-card{border-radius:var(--r);padding:14px 8px;text-align:center;background:var(--s1);border:1px solid var(--border);cursor:pointer;transition:all .2s;}
.wk-card:hover{transform:translateY(-4px);border-color:var(--border2);box-shadow:0 8px 32px rgba(0,0,0,.5), 0 0 0 1px rgba(34,197,94,.1);animation:glowPulse 1.5s ease infinite;}
.wk-card.rest{opacity:.45;cursor:default;}
.wk-card.rest:hover{transform:none;box-shadow:none;animation:none;}
...
.food-row{display:flex;align-items:center;justify-content:space-between;padding:8px 0;border-bottom:1px solid var(--border);font-size:13px;transition:background .15s;}
.food-row:hover{background:rgba(255,255,255,.02);}
.food-row:last-child{border-bottom:none;}

/* ═══════════════════════════════════════════
   EXERCISE ROW
═══════════════════════════════════════════ */
.ex-row{display:flex;align-items:flex-start;gap:10px;padding:12px 0;border-bottom:1px solid var(--border);transition:background .15s;}
.ex-row:hover{background:rgba(255,255,255,.015);}
...
/* ═══════════════════════════════════════════
   CLIENT CARD
═══════════════════════════════════════════ */
.cl-card{background:var(--s1);border:1px solid var(--border);border-radius:var(--r);padding:16px;cursor:pointer;transition:all .2s;}
.cl-card:hover{border-color:var(--green-b);transform:translateY(-2px);box-shadow:0 8px 32px rgba(0,0,0,.5);}
...
.cmp-card{background:var(--s2);border:1px solid var(--border);border-radius:12px;overflow:hidden;transition:transform .2s,border-color .2s;}
...
.sources-mini-tag{padding:3px 9px;border-radius:20px;font-size:11px;font-weight:600;border:1px solid;transition:transform .15s;}
.sources-mini-tag:hover{transform:scale(1.06);}
...
.prog-fill{height:100%;border-radius:2px;transition:width .6s cubic-bezier(.34,1.2,.64,1);}

/* ── SLIDER ── */
.slider-wrap{margin-bottom:18px;}
.slider-header{display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;}
.slider-label{font-size:13px;font-weight:600;color:var(--text);}
.slider-val{font-family:'Outfit',sans-serif;font-size:20px;font-weight:800;color:var(--green);min-width:28px;text-align:right;transition:color .2s;}
.slider{-webkit-appearance:none;appearance:none;width:100%;height:6px;border-radius:3px;background:var(--s3);outline:none;cursor:pointer;transition:opacity .15s;}
...
// ── Count-up hook for metric card numbers ──────────────────────────────────
function useCountUp(target, duration = 700) {
  ...
  useEffect(() => {
    ...
    const step = (now) => {
      ...
      const cur = ease * num;
      setDisplay(Number.isInteger(num) ? Math.round(cur) : cur.toFixed(1));
...
function MC({ label, value, color, suffix = "", flash = false }) {
  ...
  return (
    <div key={k} className={flash ? "mc flash" : "mc"}>
      {displayed != null
        ? <div className="mc-val" style={{ color }}>{displayed}<span style={{ fontSize: 13, fontWeight: 500 }}>{suffix}</span></div>
...
function AnimatedPage({ children, tabKey }) {
  const [visible, setVisible] = useState(false);
  useEffect(() => { setVisible(false); const t = setTimeout(() => setVisible(true), 30); return () => clearTimeout(t); }, [tabKey]);
  return (
    <div style={{ animation: visible ? "fadeSlideUp .38s ease both" : "none", opacity: visible ? 1 : 0 }}>
      {children}
    </div>
  );
}

// ── Staggered card list wrapper ────────────────────────────────────────────
function StaggerList({ children, baseDelay = 0, step = 60 }) {
  return (
    <>
      {Array.isArray(children)
        ? children.map((child, i) => (
          <div key={i} className="stagger" style={{ animationDelay: `${baseDelay + i * step}ms` }}>
            {child}
          </div>
        ))
...
function DailyCheckinSection({ uid, d, toast }) {
  ...
  return (
    <div className="card stagger" style={{ marginBottom: 18 }}>
      ...
      <div className="fg" style={{ marginBottom: 12 }}>
        <div className="fld">
          ...
          <select className="fsel" value={form.digestion} onChange={e => setForm(p => ({ ...p, digestion: e.target.value }))}>
            ...
            <option>Normal</option><option>Bloating</option><option>Constipation</option><option>Loose stools</option><option>Reflux / acidity</option><option>Other</option>
          </select>
        </div>
        <div className="fld">
...
function WorkoutEditor({ plan, onSave, onClose, autoSave }) {
  ...
  const triggerAutoSave = (newDays) => {
    ...
    saveTimer.current = setTimeout(() => { autoSave(newDays); setAutoSaved(true); setTimeout(() => setAutoSaved(false), 2000); }, 1000);
  };
  ...
  const removeDay = (di) => { const u = days.filter((_, i) => i !== di); setDays(u); triggerAutoSave(u); };
  ...
  return (
    <div className="ov" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal modal-lg">
        ...
        <div className="mb2">
          {days.map((day, di) => (
            <div key={di} className="stagger" style={{ animationDelay: `${di * 40}ms`, background: "var(--s2)", border: "1px solid var(--border)", borderRadius: 10, padding: 14, marginBottom: 12 }}>
              <div style={{ display: "flex", gap: 8, marginBottom: 10, alignItems: "center" }}>
                ...
                <select className="fsel" style={{ width: 130 }} value={day.type} onChange={e => updateDay(di, "type", e.target.value)}>{TYPES.map(t => <option key={t}>{t}</option>)}</select>
                  ...
                  {day.exercises.map((ex, ei) => (
                    <div key={ei} style={{ background: "var(--s1)", border: "1px solid var(--border)", borderRadius: 9, padding: 12, marginBottom: 10 }}>
                      <div style={{ display: "grid", gridTemplateColumns: "2fr 55px 80px 65px auto", gap: 6, marginBottom: 8, alignItems: "center" }}>
                        ...
                        <div style={{ paddingTop: 18 }}><button className="btn btn-d btn-xs" onClick={() => removeEx(di, ei)}>✕</button></div>
                      </div>
                      ...
                      <div>
                        ...
                        <textarea className="fta" style={{ minHeight: 60, fontSize: 12, borderColor: ex.note ? "rgba(251,191,36,.4)" : undefined, background: ex.note ? "rgba(251,191,36,.05)" : undefined }}
                          ...
                          placeholder="e.g. Use 30 degree incline. Keep elbows tucked." />
                      </div>
                    </div>
                  ))}
...
function MealEditor({ plan, onSave, onClose, clientSources, autoSave }) {
  ...
  const triggerAutoSave = (m) => {
    if (saveTimer.current) clearTimeout(saveTimer.current);
    saveTimer.current = setTimeout(() => { autoSave(m); setAutoSaved(true); setTimeout(() => setAutoSaved(false), 2000); }, 1000);
  };
    ...
    <div className="ov" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal modal-lg">
        ...
        <div className="mb2">
          ...
          {meals.map((meal, mi) => (
            <div key={mi} className="stagger" style={{ animationDelay: `${mi * 50}ms`, background: "var(--s2)", border: "1px solid var(--border)", borderRadius: 12, marginBottom: 14, overflow: "hidden" }}>
...
function ClientDash({ uid, tab, setTab, toast }) {
  ...
  if (loading) return (
    <div className="spin-wrap" style={{ minHeight: "80vh" }}>
      <div className="loading-logo">FwA</div>
      <div className="loading-text">Loading your plan...</div>
  ...
  if (tab === "checkin") return (
    <AnimatedPage tabKey="checkin">
      <div className="page">
        <div style={{ marginBottom: 22, animation: "fadeSlideUp .4s ease both" }}>
    ...
    <AnimatedPage tabKey="training">
      <div className="page">
        ...
        <div className="wk-grid" style={{ marginBottom: 20 }}>
          {workout.map((day, i) => (
            <div key={i} className={day.type === "Rest" ? "wk-card rest stagger" : "wk-card stagger"}
              style={{ borderTop: "3px solid " + (WCOLOR[day.type] || "#475569"), animationDelay: `${i * 55}ms` }}
            ...
            <div className="modal">
              ...
              <div className="mb2">
                {wModal.exercises.map((ex, i) => (
                  <div key={i} className="ex-row stagger" style={{ animationDelay: `${i * 60}ms` }}>
      ...
      <AnimatedPage tabKey="nutrition">
        <div className="page">
          ...
          <div className="g4" style={{ marginBottom: 20 }}>
            {[["Calories", n.calories, "var(--green)"], ["Protein G", n.protein, "var(--purple)"], ["Carbs G", n.carbs, "var(--orange)"], ["Fats G", n.fats, "var(--red)"]].map(([label, value, color], i) => (
              <div key={label} className="stagger" style={{ animationDelay: `${i * 80}ms` }}>
                <MC label={label} value={value} color={color} flash={!!flash[label.toLowerCase().replace(" g", "")]} />
              </div>
            ))}
          </div>
          ...
          {meals.map((meal, mi) => {
            ...
            return (
              <div key={mi} className="meal-card stagger" style={{ animationDelay: `${mi * 70}ms` }}>
          ...
          <div className="card stagger" style={{ marginBottom: 14, animationDelay: "300ms" }}>
      ...
      <AnimatedPage tabKey="sources">
        <div className="page">
          ...
          {[["protein", "Top 5 Protein Sources", "var(--purple)", "e.g. Eggs, Chicken, Paneer, Whey, Fish"], ["carbs", "Top 5 Carb Sources", "var(--orange)", "e.g. Rice, Oats, Bread, Sweet Potato, Banana"], ["fats", "Top 5 Fat Sources", "var(--red)", "e.g. Ghee, Nuts, Peanut Butter, Oil, Butter"]].map(([type, label, color, hint], ci) => (
            <div key={type} className="card stagger" style={{ marginBottom: 16, animationDelay: `${ci * 100}ms` }}>
    ...
    return (
      <AnimatedPage tabKey="photos">
        <div className="page">
          ...
          <div className="card stagger" style={{ marginBottom: 16 }}>
            <label className="upload-area">
              ...
              <div style={{ color: "var(--muted2)", fontSize: 13, marginBottom: 10 }}>Select multiple files - full quality</div>
            ...
            <div className="card stagger" style={{ animationDelay: "100ms" }}>
              ...
              <div className="photo-grid">
                {[...clientPhotos].reverse().map((p, i) => (
                  <div key={i} className="photo-item stagger" style={{ animationDelay: `${i * 40}ms` }}>
          ...
          {viewMedia && (
            <div className="ov" onClick={() => setViewMedia(null)}>
              <div style={{ maxWidth: 560, width: "100%", animation: "mIn .25s ease both" }}>
  ...
  if (tab === "comparison") {
    return (
      <AnimatedPage tabKey="comparison">
        <div className="page">
          ...
          {checkins.length < 2
            ...
            : <div className="cmp-grid">
              {checkins.map((c, i) => (
                <div key={i} className="cmp-card stagger" style={{ animationDelay: `${i * 60}ms` }}>
  ...
  // HOME
  return (
    <AnimatedPage tabKey="home-client">
      <div className="page">
        <div style={{ marginBottom: 22, animation: "fadeSlideUp .4s ease both" }}>
        ...
        <div className="g4" style={{ marginBottom: 18 }}>
          {[["Weight KG", d.weight, "var(--green)", ""], ["Waist CM", d.waist, "var(--purple)", ""], ["Body Fat %", d.bodyFat, "var(--orange)", "%"], ["Week", d.week ? "W" + d.week : null, "var(--blue)", ""]].map(([label, value, color, suffix], i) => (
            <div key={label} className="stagger" style={{ animationDelay: `${i * 80}ms` }}>
              <MC label={label} value={value} color={color} suffix={suffix} flash={!!flash[label.toLowerCase().split(" ")[0]]} />
            </div>
          ))}
        </div>
        <div className="card stagger" style={{ marginBottom: 16, animationDelay: "100ms" }}>
        ...
        <div className="card stagger" style={{ marginBottom: 16, animationDelay: "160ms", background: "var(--green-bg)", border: "1px solid var(--green-b)", cursor: "pointer" }} onClick={() => setTab("checkin")}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            ...
            <span style={{ fontSize: 22, transition: "transform .2s" }}>→</span>
          </div>
        </div>
        {checkins.length > 0 && (
          <div className="card stagger" style={{ marginBottom: 16, animationDelay: "200ms" }}>
        ...
        <div className={flash.msg ? "card flash stagger" : "card stagger"} style={{ marginBottom: 16, animationDelay: "240ms" }}>
        ...
        <div className="g4" style={{ marginBottom: 20 }}>
          {[["Calories", n.calories, "var(--green)", ""], ["Protein G", n.protein, "var(--purple)", ""], ["Carbs G", n.carbs, "var(--orange)", ""], ["Fats G", n.fats, "var(--red)", ""]].map(([label, value, color, suffix], i) => (
            <div key={label} className="stagger" style={{ animationDelay: `${280 + i * 60}ms` }}>
              <MC label={label} value={value} color={color} suffix={suffix} flash={!!flash[label.toLowerCase().split(" ")[0]]} />
            </div>
          ))}
        </div>
        ...
        <div className="card stagger" style={{ marginBottom: 20, animationDelay: "400ms" }}>
        ...
        {(d.weightHistory || []).length > 1 && (
          <div className="stagger" style={{ animationDelay: "460ms" }}>
...
function CoachDash({ coachUid, coachEmail, coachName, tab, setTab, toast }) {
  ...
  if (loading) return (
    <div className="spin-wrap" style={{ minHeight: "80vh" }}>
      <div className="loading-logo">FwA</div>
      <div className="loading-text">Loading CoachOS...</div>
      <div className="spinner" />
    </div>
  );

  if (tab === "analytics") return (
    <AnimatedPage tabKey="analytics">
      <div className="page">
        <div style={{ fontFamily: "'Outfit',sans-serif", fontSize: 22, fontWeight: 800, marginBottom: 22 }}>Analytics</div>
        <div className="g2">
          {clients.map((c, i) => (
            <div key={c.id} className="card stagger" style={{ animationDelay: `${i * 80}ms` }}>
  ...
  if (tab === "clients" && sel) {
    ...
    return (
      <AnimatedPage tabKey={selId + innerTab}>
        <div className="page">
          ...
          {innerTab === "overview" && (
            <div>
              <div className="g4" style={{ marginBottom: 16 }}>
                {[["Weight KG", sel.weight, "var(--green)", ""], ["Waist CM", sel.waist, "var(--purple)", ""], ["Body Fat %", sel.bodyFat, "var(--orange)", "%"], ["Week", "W" + sel.week, "var(--blue)", ""]].map(([label, value, color, suffix], i) => (
                  <div key={label} className="stagger" style={{ animationDelay: `${i * 70}ms` }}>
                    <MC label={label} value={value} color={color} suffix={suffix} />
                  </div>
                ))}
              </div>
              {todayCheckin && (
                <div className="card stagger" style={{ marginBottom: 14, animationDelay: "120ms" }}>
                  <div className="card-title">Today Daily Check-in <span className="bdg bdg-g">Today</span></div>
                  <div className="g3" style={{ marginBottom: 10 }}>
                    {[["Stress", todayCheckin.stressLevel, "#f87171"], ["Hunger", todayCheckin.hungerCravings, "#fb923c"], ["Training", todayCheckin.trainingPerformance, "#4ade80"], ["Nutrition", todayCheckin.nutritionAdherence, "#a78bfa"], ["Sleep", todayCheckin.sleepQuality, "#38bdf8"], ["Water", todayCheckin.waterIntake + "L", "#38bdf8"]].map(([l, v, co], i) => (
                      <div key={l} className="stagger" style={{ animationDelay: `${i * 50}ms`, background: "var(--s2)", borderRadius: 9, padding: "10px 12px", border: "1px solid var(--border)", textAlign: "center" }}>
                        <div style={{ fontFamily: "'Outfit',sans-serif", fontSize: 22, fontWeight: 800, color: co, animation: "numPop .5s .2s ease both" }}>{v}</div>
              ...
              <div className="g2 stagger" style={{ marginBottom: 14, animationDelay: "180ms" }}>
                <div className="card"><div className="card-title">Check-ins</div><div style={{ fontFamily: "'Outfit',sans-serif", fontSize: 36, fontWeight: 800, color: "var(--green)", animation: "numPop .5s .3s ease both" }}>{checkins.length}</div></div>
                <div className="card"><div className="card-title">Media</div><div style={{ fontFamily: "'Outfit',sans-serif", fontSize: 36, fontWeight: 800, color: "var(--purple)", animation: "numPop .5s .4s ease both" }}>{media.length}</div></div>
              </div>
              {(sel.weightHistory || []).length > 1 && (
                <div className="card stagger" style={{ marginBottom: 14, animationDelay: "240ms" }}>
              ...
              <div className="card stagger" style={{ marginBottom: 14, animationDelay: "280ms" }}>
          ...
          {innerTab === "daily" && (
            <div className="card stagger">
              ...
              {recentDailyCheckins.length === 0
                ? <div className="empty"><div className="empty-icon">📋</div><div className="empty-title">No daily check-ins yet</div></div>
                : recentDailyCheckins.map((c, i) => (
                  <div key={i} className="stagger" style={{ animationDelay: `${i * 60}ms`, background: "var(--s2)", borderRadius: 10, padding: 14, marginBottom: 10, border: "1px solid var(--border)" }}>
              ...
              {media.length === 0
                ...
                : <div className="photo-grid">
                  {[...media].reverse().map((p, i) => (
                    <div key={i} className="photo-item stagger" style={{ animationDelay: `${i * 35}ms` }}>
                      <div onClick={() => setViewMedia(p)} style={{ width: "100%", height: "100%" }}>
                        {p.type === "video" ? <><video src={p.url} style={{ width: "100%", height: "100%", objectFit: "cover" }} /><div className="video-badge">Video</div></> : <img src={p.url} alt="" />}
                      </div>
                      <div className="photo-label">{p.date}</div>
          ...
          {innerTab === "comparison" && (
            <div className="card stagger">
              ...
              {checkins.length < 2
                ...
                : <div className="cmp-grid">
                  {checkins.map((c, i) => (
                    <div key={i} className="cmp-card stagger" style={{ animationDelay: `${i * 55}ms` }}>
                      <div className="cmp-head" style={{ color: "var(--green)" }}>{c.week} - {c.date}</div>
                      <div className="cmp-body">
                        <div className="cmp-stat"><span style={{ color: "var(--muted)" }}>Weight</span><span style={{ fontWeight: 700, color: "var(--green)" }}>{c.weight} kg</span></div>
                        <div className="cmp-stat"><span style={{ color: "var(--muted)" }}>Waist</span><span style={{ fontWeight: 700 }}>{c.waist ? c.waist + " cm" : "-"}</span></div>
          ...
          {innerTab === "workout" && (
            <div className="card stagger">
              ...
              {(sel.workoutPlan || DEFAULT_WORKOUT).map((day, di) => (
                <div key={di} className="stagger" style={{ animationDelay: `${di * 50}ms`, background: "var(--s2)", border: "1px solid var(--border)", borderRadius: 10, padding: 12, marginBottom: 10 }}>
          ...
          {innerTab === "analytics" && (() => {
            ...
            return (
              <div>
                ...
                <div className="card stagger" style={{ marginBottom: 14, animationDelay: "80ms" }}>
                ...
                <div className="card stagger" style={{ animationDelay: "160ms" }}>
          ...
          {innerTab === "meals" && (() => {
            ...
            return (
              <div className="card stagger">
                ...
                <SourcesTablePanel sources={clientSources} />
                {mealPlan.map((meal, mi) => (
                  <div key={mi} className="meal-card stagger" style={{ animationDelay: `${mi * 60}ms` }}>
          ...
          {viewMedia && (
            <div className="ov" onClick={() => setViewMedia(null)}>
              <div style={{ maxWidth: 520, width: "100%", animation: "mIn .25s ease both" }}>
  ...
  if (tab === "clients") return (
    <AnimatedPage tabKey="clients-list">
      <div className="page">
            ...
            <table className="tbl">
              ...
              <tbody>
                {clients.map((c, i) => {
                  ...
                  return (
                    <tr key={c.id} style={{ animation: `fadeIn .3s ${i * 50}ms ease both` }}>
  ...
  // COACH HOME
  return (
    <AnimatedPage tabKey="coach-home">
      <div className="page">
        <div style={{ marginBottom: 24, animation: "fadeSlideUp .4s ease both" }}>
        ...
        <div className="g4" style={{ marginBottom: 24 }}>
          {[["Total Clients", clients.length, "var(--green)"], ["Total Check-ins", clients.reduce((a, c) => a + (c.checkIns || []).length, 0), "var(--blue)"], ["Daily Logs Today", clients.filter(c => (c.dailyCheckins || []).some(dc => dc.date === new Date().toLocaleDateString("en-IN"))).length, "var(--purple)"], ["Need Attention", clients.filter(c => !c.coachMessage).length, "var(--orange)"]].map(([l, v, co], i) => (
            <div key={l} className="stagger" style={{ animationDelay: `${i * 80}ms` }}>
              <div className="mc"><div className="mc-val" style={{ color: co }}>{v}</div><div className="mc-label">{l}</div></div>
            </div>
          ))}
        </div>
        {clients.length === 0
          ? <div className="card stagger" style={{ animationDelay: "160ms" }}><div className="empty"><div className="empty-icon">🚀</div><div className="empty-title">Ready to go!</div><div className="empty-desc">Add your first client to get started.</div><button className="btn btn-p" style={{ padding: "11px 24px" }} onClick={() => setTab("clients")}>+ Add First Client</button></div></div>
          : <>
            ...
            <div className="ga">
              {clients.map((c, i) => {
                const todayDaily = (c.dailyCheckins || []).find(dc => dc.date === new Date().toLocaleDateString("en-IN"));
                return (
                  <div key={c.id} className="cl-card stagger" style={{ animationDelay: `${i * 70}ms` }} onClick={() => { setTab("clients"); setSelId(c.id); setInnerTab("overview"); }}>
...
function LoginScreen({ onLogin, onSetup, coachExists }) {
  ...
  return (
    <div className="auth-wrap"><div className="auth-card">
      ...
      {err && <div className="alert alert-e" style={{ animation: "fadeIn .2s ease both" }}>{err}</div>}
  ...
  if (authLoading) return (
    <div style={{ background: "var(--bg)", minHeight: "100vh" }}>
      ...
      <div className="spin-wrap" style={{ minHeight: "100vh" }}>
        <div className="loading-logo">FwA</div>
        <div className="loading-text">Loading Fit with Ankit...</div>
        <div className="spinner" />
      </div>
    </div>
  );
