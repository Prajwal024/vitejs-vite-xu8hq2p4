...
const DEFAULT_WORKOUT = [
  { day: "Monday", type: "Push", exercises: [{ name: "Bench Press", sets: 4, reps: "8-10", rest: "90s" }, { name: "Overhead Press", sets: 3, reps: "10-12", rest: "75s" }, { name: "Tricep Pushdowns", sets: 3, reps: "12-15", rest: "60s" }] },
  { day: "Tuesday", type: "Pull", exercises: [{ name: "Pull-ups", sets: 4, reps: "8-10", rest: "90s" }, { name: "Barbell Row", sets: 4, reps: "8-10", rest: "90s" }, { name: "Hammer Curls", sets: 3, reps: "12-15", rest: "60s" }] },
  { day: "Wednesday", type: "Rest", exercises: [] },
  { day: "Thursday", type: "Legs", exercises: [{ name: "Back Squat", sets: 4, reps: "6-8", rest: "120s" }, { name: "Romanian DL", sets: 4, reps: "8-10", rest: "90s" }, { name: "Leg Curl", sets: 3, reps: "12-15", rest: "60s" }] },
  { day: "Friday", type: "Push", exercises: [{ name: "Incline DB Press", sets: 4, reps: "10-12", rest: "90s" }, { name: "Lateral Raises", sets: 4, reps: "15-20", rest: "45s" }, { name: "Skull Crushers", sets: 3, reps: "12-15", rest: "60s" }] },
  { day: "Saturday", type: "Pull", exercises: [{ name: "Deadlift", sets: 4, reps: "4-6", rest: "120s" }, { name: "Cable Row", sets: 3, reps: "10-12", rest: "75s" }, { name: "Barbell Curl", sets: 3, reps: "10-12", rest: "60s" }] },
  { day: "Sunday", type: "Rest", exercises: [] },
];

const DEFAULT_MEALS = [
  { name: "Breakfast", time: "7:00 AM", items: [{ food: "Oats", amount: "80g", protein: 10, carbs: 54, fats: 5, cal: 300 }, { food: "Banana", amount: "1 medium", protein: 1, carbs: 27, fats: 0, cal: 105 }, { food: "Whey Protein", amount: "1 scoop", protein: 25, carbs: 3, fats: 2, cal: 130 }] },
  { name: "Lunch", time: "1:00 PM", items: [{ food: "Chicken Breast", amount: "200g", protein: 46, carbs: 0, fats: 4, cal: 220 }, { food: "Brown Rice", amount: "150g", protein: 4, carbs: 47, fats: 1, cal: 210 }, { food: "Broccoli", amount: "100g", protein: 3, carbs: 7, fats: 0, cal: 35 }] },
  { name: "Pre-Workout", time: "5:00 PM", items: [{ food: "Banana", amount: "1 large", protein: 1, carbs: 31, fats: 0, cal: 120 }, { food: "Peanut Butter", amount: "2 tbsp", protein: 8, carbs: 6, fats: 16, cal: 190 }] },
  { name: "Dinner", time: "8:00 PM", items: [{ food: "Eggs", amount: "4 whole", protein: 24, carbs: 2, fats: 20, cal: 280 }, { food: "Sweet Potato", amount: "200g", protein: 3, carbs: 40, fats: 0, cal: 172 }, { food: "Olive Oil", amount: "1 tbsp", protein: 0, carbs: 0, fats: 14, cal: 120 }] },
];
...
.tab-item{flex:1;padding:7px 6px;border-radius:8px;border:none;background:transparent;cursor:pointer;font-family:'DM Sans',sans-serif;font-size:12px;font-weight:600;color:var(--muted);transition:all .15s;min-width:80px}
...
.spin-wrap{display:flex;align-items:center;justify-content:min-height:260px;gap:10px;color:var(--muted);font-size:13px;justify-content:center;min-height:260px}
...
.checkin-row{display:flex;align-items:center;justify-content:space-between;padding:10px 14px;border-bottom:1px solid var(--border);font-size:13px}
...
.photo-label{position:absolute;bottom:0;left:0;right:0;background:linear-gradient(transparent,rgba(0,0,0,.7));padding:14px 8px 8px;color:#fff;font-size:10px;font-weight:600}
...
function ClientDash({ uid, tab, setTab, toast }) {
  ...
  useEffect(() => {
    const unsub = onSnapshot(doc(db, "clients", uid), snap => {
      ...
      if (prevRef.current) {
        ...
        ["calories", "protein", "carbs", "fats"].forEach(k => { if (pn[k] !== n[k]) changed[k] = true; });
  ...
  const logStats = async () => {
    ...
    toast("✓ Check-in logged: " + lw + "kg", "success");
    setLw(""); setLwa(""); setSaving(false);
  };
  ...
  const uploadPhoto = async (file) => {
    ...
    reader.onload = async (e) => {
      ...
      await updateDoc(doc(db, "clients", uid), { photos: [...photos, newPhoto] });
      toast("📸 Photo uploaded! Coach can see it now.", "success");
    };
    reader.readAsDataURL(file);
  };

  if (loading) return <div className="spin-wrap"><div className="spinner" /><span>Loading your plan...</span></div>;
  if (!d) return <div className="spin-wrap">No data found. Contact your coach.</div>;
  ...
  if (tab === "training") return (
    <div className="page">
      <div style={{ marginBottom: 22 }}>
        ...
        {flash.workout && <span className="nbadge" style={{ marginTop: 8 }}>✨ Updated by coach</span>}
      </div>
            ...
            <div className="mb2">
              {wModal.exercises.map((ex, i) => (
                <div key={i} className="ex-row">
                  ...
                  <div style={{ flex: 1 }}><div style={{ fontWeight: 600, fontSize: 13 }}>{ex.name}</div><div style={{ fontSize: 11, color: "var(--muted)" }}>Rest: {ex.rest}</div></div>
                  <div style={{ textAlign: "right" }}><div style={{ fontWeight: 700, fontSize: 13, color: WCOLOR[wModal.type] }}>{ex.sets} × {ex.reps}</div></div>
                </div>
              ))}
            </div>
          </div>
  ...
  if (tab === "nutrition") return (
    <div className="page">
      <div style={{ marginBottom: 18 }}>
        ...
        {flash.meals && <span className="nbadge" style={{ marginTop: 8 }}>✨ Meal plan updated</span>}
      </div>
            ...
            {meal.items.map((item, ii) => (
              <div key={ii} className="food-row">
                ...
                <div style={{ display: "flex", gap: 10, fontSize: 12 }}>
                  ...
                  <span style={{ color: "var(--green)", fontWeight: 600 }}>{item.cal} cal</span>
                </div>
              </div>
            ))}
          </div>
  ...
  if (tab === "photos") return (
    <div className="page">
      <div style={{ marginBottom: 20 }}>
        ...
        <div style={{ color: "var(--muted)", fontSize: 13, marginTop: 5 }}>Your coach can see all photos you upload</div>
      </div>
      <div className="card" style={{ marginBottom: 16 }}>
        <label style={{ border: "2px dashed var(--border2)", borderRadius: 12, padding: 32, textAlign: "center", cursor: "pointer", display: "block", background: "var(--s2)" }}>
          <input type="file" accept="image/*" style={{ display: "none" }}
            onChange={e => { const f = e.target.files[0]; if (f) uploadPhoto(f); }} />
          <div style={{ fontWeight: 600, fontSize: 14, color: "var(--green)", marginBottom: 4 }}>📤 Click to upload photo</div>
          <div style={{ color: "var(--muted)", fontSize: 12 }}>JPG or PNG · visible to your coach instantly</div>
        </label>
      </div>
      {clientPhotos.length > 0 ? (
        <div className="card">
          <div className="card-title">Your Photos ({clientPhotos.length})</div>
          <div className="photo-grid">
            {clientPhotos.map((p, i) => (
              <div key={i} className="photo-item" onClick={() => setViewPhoto(p)}>
                <img src={p.data} alt="" />
        ...
        <div className="card"><div className="empty"><div className="empty-icon">📸</div><div className="empty-title">No photos yet</div><div className="empty-desc">Upload your first progress photo!</div></div></div>
      )}
      {viewPhoto && (
        <div className="ov" onClick={() => setViewPhoto(null)}>
          <div style={{ maxWidth: 500, width: "100%" }}>
            <img src={viewPhoto.data} alt="" style={{ width: "100%", borderRadius: 16 }} />
            <div style={{ textAlign: "center", marginTop: 10, color: "var(--muted)", fontSize: 13 }}>{viewPhoto.date} · {viewPhoto.time}</div>
          </div>
        </div>
      )}
    </div>
  ...
  return (
    <div className="page">
      ...
      {(d.checkIns || []).length > 0 && (
        <div className="card" style={{ marginBottom: 16 }}>
          <div className="card-title">📊 My Check-in History</div>
          <table className="tbl">
            <thead><tr><th>Week</th><th>Date</th><th>Weight</th><th>Waist</th></tr></thead>
            <tbody>
              {[...(d.checkIns || [])].reverse().slice(0, 10).map((c, i) => (
                <tr key={i}>
                  <td><span className="bdg bdg-g">{c.week}</span></td>
                  <td style={{ color: "var(--muted)" }}>{c.date} · {c.time}</td>
      ...
      <div className={flash.msg ? "card flash" : "card"} style={{ marginBottom: 16 }}>
        <div className="card-title" style={{ display: "flex", alignItems: "center" }}>
          💬 Coach's Message {flash.msg && <span className="nbadge">✨ New</span>}
        </div>
            ...
            <div className="mb2">
              {wModal.exercises.map((ex, i) => (
                <div key={i} className="ex-row">
                  <div className="ex-num">{i + 1}</div>
                  <div style={{ flex: 1 }}><div style={{ fontWeight: 600, fontSize: 13 }}>{ex.name}</div><div style={{ fontSize: 11, color: "var(--muted)" }}>Rest: {ex.rest}</div></div>
                  <div style={{ textAlign: "right" }}><div style={{ fontWeight: 700, fontSize: 13 }}>{ex.sets} × {ex.reps}</div></div>
                </div>
              ))}
            </div>
          </div>
...
function WorkoutEditor({ plan, onSave, onClose }) {
  ...
  const addEx = (di) => setDays(d => d.map((day, i) => i === di ? { ...day, exercises: [...day.exercises, { name: "", sets: 3, reps: "10-12", rest: "60s" }] } : day));
  const removeEx = (di, ei) => setDays(d => d.map((day, i) => i === di ? { ...day, exercises: day.exercises.filter((_, j) => j !== ei) } : day));
  return (
    <div className="ov" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal modal-lg">
        <div className="mh"><div><div className="mt">✏️ Edit Workout Plan</div><div className="ms">Changes save to client instantly</div></div><button className="xbtn" onClick={onClose}>✕</button></div>
        <div className="mb2">
          {days.map((day, di) => (
            <div key={di} style={{ background: "var(--s2)", border: "1px solid var(--border)", borderRadius: 10, padding: 14, marginBottom: 12 }}>
              <div style={{ display: "flex", gap: 8, marginBottom: 10, alignItems: "center" }}>
                ...
                <select className="fsel" style={{ width: 120 }} value={day.type} onChange={e => updateDay(di, "type", e.target.value)}>
              ...
              {day.type !== "Rest" && (
                <>
                  {day.exercises.map((ex, ei) => (
                    <div key={ei} style={{ display: "grid", gridTemplateColumns: "2fr 60px 80px 70px auto", gap: 6, marginBottom: 6, alignItems: "center" }}>
                      ...
                      <input className="fi" value={ex.reps} onChange={e => updateEx(di, ei, "reps", e.target.value)} placeholder="Reps" />
                      <input className="fi" value={ex.rest} onChange={e => updateEx(di, ei, "rest", e.target.value)} placeholder="Rest" />
...
function MealEditor({ plan, onSave, onClose }) {
  ...
  const addItem = (mi) => setMeals(m => m.map((meal, i) => i === mi ? { ...meal, items: [...meal.items, { food: "", amount: "", protein: 0, carbs: 0, fats: 0, cal: 0 }] } : meal));
  ...
  return (
    <div className="ov" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal modal-lg">
        <div className="mh"><div><div className="mt">🥩 Edit Meal Plan</div><div className="ms">Changes save to client instantly</div></div><button className="xbtn" onClick={onClose}>✕</button></div>
        <div className="mb2">
          {meals.map((meal, mi) => (
            <div key={mi} style={{ background: "var(--s2)", border: "1px solid var(--border)", borderRadius: 10, padding: 14, marginBottom: 12 }}>
              <div style={{ display: "flex", gap: 8, marginBottom: 10, alignItems: "center" }}>
                ...
                <button className="btn btn-d btn-sm" onClick={() => removeMeal(mi)}>✕</button>
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 50px 50px 50px 60px auto", gap: 5, marginBottom: 6 }}>
                {["Food", "Amount", "P(g)", "C(g)", "F(g)", "Cal", ""].map((h, i) => <div key={i} className="fl">{h}</div>)}
              </div>
              {meal.items.map((item, ii) => (
                <div key={ii} style={{ display: "grid", gridTemplateColumns: "2fr 1fr 50px 50px 50px 60px auto", gap: 5, marginBottom: 6, alignItems: "center" }}>
                  ...
                  <input className="fi" type="number" value={item.protein} onChange={e => updateItem(mi, ii, "protein", e.target.value)} />
                  <input className="fi" type="number" value={item.carbs} onChange={e => updateItem(mi, ii, "carbs", e.target.value)} />
                  <input className="fi" type="number" value={item.fats} onChange={e => updateItem(mi, ii, "fats", e.target.value)} />
                  <input className="fi" type="number" value={item.cal} onChange={e => updateItem(mi, ii, "cal", e.target.value)} />
...
function CoachDash({ coachUid, coachEmail, coachName, tab, setTab, toast }) {
  ...
  const [viewPhoto, setViewPhoto] = useState(null);
  const [nc, setNc] = useState({ name: "", email: "", password: "", phone: "", phase: "Cut Phase 1", week: "1", calories: "2200", protein: "160", carbs: "240", fats: "70" });
  ...
  const saveMeals = async (plan) => {
    const calories = plan.reduce((a, m) => a + m.items.reduce((b, i) => b + (i.cal || 0), 0), 0);
    const protein = plan.reduce((a, m) => a + m.items.reduce((b, i) => b + (i.protein || 0), 0), 0);
    const carbs = plan.reduce((a, m) => a + m.items.reduce((b, i) => b + (i.carbs || 0), 0), 0);
    const fats = plan.reduce((a, m) => a + m.items.reduce((b, i) => b + (i.fats || 0), 0), 0);
    await updateDoc(doc(db, "clients", selId), { mealPlan: plan, nutrition: { calories, protein, carbs, fats } });
  ...
  const addClient = async () => {
    ...
    try {
      ...
      await setDoc(doc(db, "clients", clientUid), {
        ...
        nutrition: { calories: parseInt(nc.calories) || 2000, protein: parseInt(nc.protein) || 150, carbs: parseInt(nc.carbs) || 200, fats: parseInt(nc.fats) || 60 },
      ...
      setShowAdd(false);
      setNc({ name: "", email: "", password: "", phone: "", phase: "Cut Phase 1", week: "1", calories: "2200", protein: "160", carbs: "240", fats: "70" });
      toast("✅ " + nc.name + " added! Share the app link + their login via WhatsApp.", "success");
    } catch (err) {
  ...
  if (tab === "analytics") return (
    <div className="page">
      ...
      {clients.length === 0
        ? <div className="card"><div className="empty"><div className="empty-icon">📊</div><div className="empty-title">No clients yet</div></div></div>
        : <div className="g2">
          {clients.map(c => (
    ...
    return (
      <div className="page">
        ...
        <div className="tab-bar">
          {[["overview","📊 Overview"],["checkins","📋 Check-ins"],["photos","📸 Photos"],["message","📨 Message"],["nutrition","🥩 Macros"],["meals","🍽️ Meals"],["workout","💪 Workout"],["phase","📋 Phase"]].map(([k, l]) => (
          ...
          <div>
            ...
            <div className="g2" style={{ marginBottom: 14 }}>
              <div className="card">
                ...
                <div style={{ fontFamily: "'Outfit',sans-serif", fontSize: 36, fontWeight: 800, color: "var(--green)" }}>{checkins.length}</div>
                <div style={{ fontSize: 12, color: "var(--muted)", marginTop: 4 }}>Last: {checkins.length > 0 ? checkins[checkins.length - 1].date : "None"}</div>
              </div>
              <div className="card">
                <div className="card-title">📸 Progress Photos</div>
                <div style={{ fontFamily: "'Outfit',sans-serif", fontSize: 36, fontWeight: 800, color: "var(--purple)" }}>{clientPhotos.length}</div>
                <div style={{ fontSize: 12, color: "var(--muted)", marginTop: 4 }}>Photos uploaded</div>
              </div>
            </div>
        ...
        {innerTab === "checkins" && (
          <div className="card">
            <div className="card-title">📋 Full Check-in History for {sel.name}</div>
            {checkins.length === 0
              ? <div className="empty"><div className="empty-icon">📋</div><div className="empty-title">No check-ins yet</div><div className="empty-desc">Client hasn't logged any stats yet.</div></div>
              : <table className="tbl">
                <thead><tr><th>Week</th><th>Date & Time</th><th>Weight</th><th>Waist</th><th>Change</th></tr></thead>
                <tbody>
                  {[...checkins].reverse().map((c, i, arr) => {
                    ...
                    return (
                      <tr key={i}>
                        ...
                        <td>
                          {change !== null && (
                            <span className={"bdg " + (parseFloat(change) < 0 ? "bdg-g" : parseFloat(change) > 0 ? "bdg-r" : "bdg-p")}>
        ...
        {innerTab === "photos" && (
          <div className="card">
            <div className="card-title">📸 Progress Photos — {sel.name} ({clientPhotos.length} photos)</div>
            {clientPhotos.length === 0
              ? <div className="empty"><div className="empty-icon">📸</div><div className="empty-title">No photos yet</div><div className="empty-desc">Client hasn't uploaded any photos yet.</div></div>
              : <div className="photo-grid">
        ...
        {innerTab === "message" && (
          <div className="card">
            ...
            <div className="fld"><div className="fl">New Message</div><textarea className="fta" placeholder={"Great work " + sel.name + "! Here's your update..."} value={msgText} onChange={e => setMsgText(e.target.value)} /></div>
            <div className="alert alert-w">⚡ Client sees this instantly!</div>
            <button className="btn btn-p" onClick={sendMessage} disabled={sendingMsg || !msgText.trim()}>{sendingMsg ? "Sending..." : "📤 Send to " + sel.name}</button>
          </div>
        )}
        ...
        {innerTab === "nutrition" && (
          <div className="card">
            <div className="card-title">🥩 Daily Macro Targets for {sel.name}</div>
            <div className="alert alert-w">⚡ Each change syncs to {sel.name} instantly!</div>
            <div className="fg">
              {[["Calories (kcal)", "calories", "var(--green)"], ["Protein (g)", "protein", "var(--purple)"], ["Carbs (g)", "carbs", "var(--orange)"], ["Fats (g)", "fats", "var(--red)"]].map(([l, k, co]) => (
                <div key={k} className="fld">
                  <div className="fl" style={{ color: co }}>{l}</div>
                  <input className="fi" type="number" defaultValue={n[k]} key={selId + k + n[k]} onBlur={e => updateN(k, e.target.value)} />
                </div>
              ))}
            </div>
            <div className="alert alert-g"><strong>Current:</strong> {n.calories} kcal · {n.protein}g P · {n.carbs}g C · {n.fats}g F</div>
          </div>
        )}
        ...
        {innerTab === "meals" && (
          <div className="card">
            <div className="card-title" style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              ...
              <button className="btn btn-p btn-sm" onClick={() => setShowMealEditor(true)}>✏️ Edit Meals</button>
            </div>
            <div className="alert alert-w">⚡ Click "Edit Meals" to fully customize every meal!</div>
        ...
        {innerTab === "workout" && (
          <div className="card">
            <div className="card-title" style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              ...
              <button className="btn btn-p btn-sm" onClick={() => setShowWorkoutEditor(true)}>✏️ Edit Workouts</button>
            </div>
            <div className="alert alert-w">⚡ Click "Edit Workouts" to add/remove exercises!</div>
            {(sel.workoutPlan || DEFAULT_WORKOUT).map((day, di) => (
              <div key={di} style={{ background: "var(--s2)", border: "1px solid var(--border)", borderRadius: 10, padding: 12, marginBottom: 10 }}>
                ...
                {day.type !== "Rest" && day.exercises.map((ex, ei) => (
                  <div key={ei} style={{ display: "flex", justifyContent: "space-between", padding: "5px 0", borderBottom: "1px solid var(--border)", fontSize: 13 }}>
                    <span>{ex.name}</span>
        ...
        {innerTab === "phase" && (
          <div className="card">
            ...
            <div className="alert alert-w">⚡ All changes sync to client instantly.</div>
            <div className="fg">
              <div className="fld">
              ...
              <div className="fld">
                ...
                <input className="fi" type="number" min="1" max="52" value={sel.week} onChange={e => update("week", parseInt(e.target.value) || 1)} />
              </div>
              <div className="fld">
                <div className="fl">Body Fat %</div>
                <input className="fi" type="number" step="0.1" defaultValue={sel.bodyFat || ""} placeholder="18.5" onBlur={e => update("bodyFat", parseFloat(e.target.value) || null)} />
              </div>
              <div className="fld">
        ...
        {viewPhoto && (
          <div className="ov" onClick={() => setViewPhoto(null)}>
            <div style={{ maxWidth: 500, width: "100%" }}>
              <img src={viewPhoto.data} alt="" style={{ width: "100%", borderRadius: 16 }} />
              <div style={{ textAlign: "center", marginTop: 10, color: "var(--muted2)", fontSize: 13 }}>{sel.name} · {viewPhoto.date} · {viewPhoto.time}</div>
              <div style={{ textAlign: "center", marginTop: 8 }}><button className="btn btn-s btn-sm" onClick={() => setViewPhoto(null)}>Close</button></div>
            </div>
          </div>
        )}
      </div>
  ...
  if (tab === "clients") return (
    <div className="page">
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
        <div>
          <div style={{ fontFamily: "'Outfit',sans-serif", fontSize: 22, fontWeight: 800 }}>👥 My Clients</div>
          <div style={{ color: "var(--muted)", fontSize: 13, marginTop: 3 }}>{clients.length} active</div>
        </div>
        <button className="btn btn-p" onClick={() => setShowAdd(true)}>+ Add Client</button>
      </div>
      {clients.length === 0
        ? <div className="card"><div className="empty"><div className="empty-icon">👤</div><div className="empty-title">No clients yet</div><div className="empty-desc">Add your first client to get started.</div><button className="btn btn-p" onClick={() => setShowAdd(true)}>+ Add Client</button></div></div>
        : <div className="card">
          <table className="tbl">
            <thead><tr><th>Client</th><th>Phase</th><th>Weight</th><th>Check-ins</th><th>Photos</th><th>Message</th><th>Actions</th></tr></thead>
            <tbody>
              {clients.map(c => (
                <tr key={c.id}>
                  <td>
                    <div style={{ display: "flex", alignItems: "center", gap: 9 }}>
                      <div className="av av-sm av-g">{c.avatar}</div>
                      <div><div style={{ fontWeight: 600 }}>{c.name}</div><div style={{ fontSize: 11, color: "var(--muted)" }}>{c.email}</div></div>
                    </div>
                  </td>
                  <td><span className="bdg bdg-g">{c.phase}</span></td>
                  <td style={{ fontWeight: 600 }}>{c.weight ? c.weight + "kg" : <span style={{ color: "var(--muted)" }}>—</span>}</td>
                  <td><span className="bdg bdg-p">{(c.checkIns || []).length} logs</span></td>
                  <td><span className="bdg bdg-o">{(c.photos || []).length} photos</span></td>
                  <td>{c.coachMessage ? <span style={{ color: "var(--green)", fontSize: 12, fontWeight: 600 }}>✓ Sent</span> : <span style={{ color: "var(--yellow)", fontSize: 12, fontWeight: 600 }}>⚠ Pending</span>}</td>
                  <td>
                    <div style={{ display: "flex", gap: 5 }}>
                      ...
                      <button className="btn btn-s btn-sm" onClick={() => { setSelId(c.id); setInnerTab("photos"); }}>📸</button>
                    </div>
                  </td>
                </tr>
              ))}
        ...
        <div className="ov" onClick={e => e.target === e.currentTarget && setShowAdd(false)}>
          <div className="modal">
            ...
            <div className="mb2">
              ...
              <div className="sec-lbl" style={{ marginTop: 4 }}>Login Credentials</div>
              <div className="fg">
              ...
              <div className="alert alert-b">📲 Share app URL + email + password with client via WhatsApp!</div>
              ...
              <div className="fg">
                ...
                <div className="fld"><div className="fl">Starting Week</div><input className="fi" type="number" min="1" value={nc.week} onChange={e => setNc(p => ({ ...p, week: e.target.value }))} /></div>
              </div>
              <div className="sec-lbl" style={{ marginTop: 4 }}>Starting Macros</div>
              <div className="fg">
                {[["Calories", "calories"], ["Protein (g)", "protein"], ["Carbs (g)", "carbs"], ["Fats (g)", "fats"]].map(([l, k]) => (
                  <div key={k} className="fld"><div className="fl">{l}</div><input className="fi" type="number" value={nc[k]} onChange={e => setNc(p => ({ ...p, [k]: e.target.value }))} /></div>
                ))}
              </div>
              <div style={{ display: "flex", gap: 10 }}>
  ...
  return (
    <div className="page">
      ...
      <div className="g4" style={{ marginBottom: 24 }}>
        {[["Total Clients", clients.length, "var(--green)"], ["Total Check-ins", clients.reduce((a, c) => a + (c.checkIns || []).length, 0), "var(--blue)"], ["Photos Uploaded", clients.reduce((a, c) => a + (c.photos || []).length, 0), "var(--purple)"], ["Need Attention", clients.filter(c => !c.coachMessage).length, "var(--orange)"]].map(([l, v, co]) => (
          <div key={l} className="mc"><div className="mc-val" style={{ color: co }}>{v}</div><div className="mc-label">{l}</div></div>
        ))}
      </div>
      {clients.length === 0
        ? <div className="card"><div className="empty"><div className="empty-icon">🚀</div><div className="empty-title">You're all set!</div><div className="empty-desc">Add your first client to get started.</div><button className="btn btn-p" style={{ fontSize: 14, padding: "11px 24px" }} onClick={() => setTab("clients")}>+ Add First Client</button></div></div>
        : <>
            ...
            {clients.map(c => (
              <div key={c.id} className="cl-card" onClick={() => { setTab("clients"); setSelId(c.id); setInnerTab("overview"); }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 10 }}>
                  ...
                  <div style={{ padding: "8px 10px", background: "var(--s2)", borderRadius: 9, border: "1px solid var(--border)" }}>
                    <div style={{ fontSize: 13, fontWeight: 700, color: "var(--purple)", fontFamily: "'Outfit',sans-serif" }}>{(c.checkIns || []).length} logs</div>
                    <div style={{ fontSize: 10, color: "var(--muted)", fontWeight: 600, textTransform: "uppercase", marginTop: 2 }}>Check-ins</div>
                  </div>
                </div>
                <div style={{ display: "flex", gap: 6, marginBottom: 10 }}>
                  <span className="bdg bdg-o" style={{ fontSize: 10 }}>📸 {(c.photos || []).length} photos</span>
                  <span className="bdg bdg-p" style={{ fontSize: 10 }}>{c.phase?.split(" ").slice(0,2).join(" ")}</span>
                </div>
  ...
  return (
    <div className="auth-wrap">
      <div className="auth-card">
        ...
        <div className="fld"><div className="fl">Password</div><input className="fi" type="password" placeholder="••••••••" value={pw} onChange={e => setPw(e.target.value)} onKeyDown={e => e.key === "Enter" && login()} /></div>
        {err && <div className="alert alert-e">{err}</div>}
        <button className="auth-btn" onClick={login} disabled={ld}>{ld ? "Signing in..." : "Sign In →"}</button>
        {/* Only show setup link if NO coach exists yet */}
        {!coachExists && (
          <div className="auth-switch">First time setup? <button onClick={onSetup}>Create coach account</button></div>
        )}
        {coachExists && (
          <div style={{ textAlign: "center", marginTop: 14, fontSize: 11, color: "var(--muted)", padding: "10px", background: "var(--s2)", borderRadius: 10, border: "1px solid var(--border)" }}>
            🔒 This platform is managed by a coach. Contact your coach for login credentials.
          </div>
        )}
      </div>
    </div>
...
function SetupScreen({ onDone }) {
  ...
  const create = async () => {
    ...
    try {
      ...
      // Mark that a coach exists in a global settings doc
      ...
      setErr(e.code === "auth/email-already-in-use" ? "Email already registered. Log in instead." : e.message);
    }
    setLd(false);
  };
  return (
    <div className="auth-wrap">
      <div className="auth-card">
        ...
        <div className="fld"><div className="fl">Your Full Name</div><input className="fi" placeholder="Coach Ankit Ingle" value={f.name} onChange={e => setF(p => ({ ...p, name: e.target.value }))} /></div>
        <div className="fld"><div className="fl">Your Email</div><input className="fi" type="email" placeholder="coach@email.com" value={f.email} onChange={e => setF(p => ({ ...p, email: e.target.value }))} /></div>
...
export default function App() {
  ...
  // Check if coach account already exists
  useEffect(() => {
    const checkCoach = async () => {
      try {
        const settingsSnap = await getDoc(doc(db, "settings", "app"));
        if (settingsSnap.exists() && settingsSnap.data().coachExists) {
          setCoachExists(true);
        }
      } catch (e) {}
    };
  ...
  if (!user) return (
    <div><style>{CSS}</style>
      {screen === "setup"
        ? <SetupScreen onDone={u => { setUser(u); setCoachExists(true); setScreen("login"); }} />
        : <LoginScreen onLogin={setUser} onSetup={() => setScreen("setup")} coachExists={coachExists} />}
  ...
  const tabs = isCoach
    ...
    : [["home", "🏠 Home"], ["nutrition", "🥩 Nutrition"], ["training", "🏋️ Training"], ["photos", "📸 Photos"]];
